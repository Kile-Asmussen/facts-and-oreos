...
impl Build {
    pub fn new() -> Build {
        Build {
            out_dir: env::var_os("OUT_DIR").map(|s| PathBuf::from(s).join("flua-build")),
    ...
    pub fn build(&self) -> Artifacts {
        ...
        let target = self.target.as_deref().expect("TARGET not set");
        let host = self.host.as_deref().expect("HOST not set");
        let out_dir = self.out_dir.as_ref().expect("OUT_DIR not set");
        let lib_dir = out_dir.join("lib");
        let include_dir = out_dir.join("include");

        // The flua submodule must be initialised. A missing/empty src dir means
        // the user cloned without --recurse-submodules.
        let source_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("flua/src");

        if !source_dir.join("lua.h").exists() {
            panic!(
                "\n\nflua-src/flua submodule is not initialised. Run:\n\
            ...
            || target.contains("openbsd")
        {
            ...
            panic!("don't know how to build FLua for target: {target}");
        }
        ...
        // lua.c and luac.c are standalone programs, not library sources.
        // LuaCPPUtilities.hpp and override_printf.h are not compiled directly.
        config
            .include(&source_dir)
            .flag("-w")
        ...
        Artifacts {
            ...
            cpp_stdlib: Self::cpp_link_stdlib(target),
        }
    }

    fn cpp_link_stdlib(target: &str) -> Option<String> {
