//! Lua 5.3+ API compatibility shims for Lua 5.2/FLua.
//!
//! Ported from mlua-sys src/lua52/compat.rs (based on keplerproject/lua-compat-5.3).
...
pub unsafe fn lua_rotate(L: *mut lua_State, mut idx: c_int, mut n: c_int) {
    ...
    if n > 0 && n < n_elems {
        luaL_checkstack(L, 2, c"not enough stack slots available".as_ptr());
        n = n_elems - n;
...
pub unsafe fn lua_seti(L: *mut lua_State, mut idx: c_int, n: lua_Integer) {
    luaL_checkstack(L, 1, c"not enough stack slots available".as_ptr());
    idx = lua_absindex(L, idx);
    lua_pushinteger(L, n);
    lua_insert(L, -2);
    lua_settable(L, idx);
...
pub unsafe fn luaL_tolstring(L: *mut lua_State, mut idx: c_int, len: *mut usize) -> *const c_char {
    ...
    if luaL_callmeta(L, idx, c"__tostring".as_ptr()) == 0 {
        match lua_type(L, idx) {
            LUA_TNIL => lua_pushliteral(L, c"nil"),
            ...
            t => {
                ...
                lua_pushfstring(L, c"%s: %p".as_ptr(), name, lua_topointer(L, idx));
                if tt != LUA_TNIL {
                    lua_replace(L, -2);
                }
            }
...
pub unsafe fn luaL_requiref(L: *mut lua_State, modname: *const c_char, openf: lua_CFunction, glb: c_int) {
    luaL_checkstack(L, 3, c"not enough stack slots available".as_ptr());
